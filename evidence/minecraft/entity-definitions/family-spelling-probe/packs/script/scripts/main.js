// Family-token spelling probe.
//
//   scriptevent probe:run <label>
//
// Spawns probe:spelling (declaring an underscore, a hyphenated, and a dotted family token
// together), plus one solo entity per token, then reads each token back through every family
// read path. Every line goes to console.warn so a dedicated server collects it from the
// content log with no client attached.
//
// The harness also drives a console-side selector check:
//   execute if entity @e[family=<t>] run scriptevent probe:hit <t>
//   scriptevent probe:checked <t>
// probe:hit and probe:checked are echoed here so the console path lands in the content log.
...
const run = async (label) => {
  ...
  await wait(5);
  const { d, at } = arena();
  const spelling = read(() => d.spawnEntity("probe:spelling", at(-3))).v;
  const under = read(() => d.spawnEntity("probe:under", at(-1))).v;
  const hyphen = read(() => d.spawnEntity("probe:hyphen", at(1))).v;
  const dot = read(() => d.spawnEntity("probe:dot", at(3))).v;
  await wait(10);
  inspect(`${label}/spelling`, spelling);
  inspect(`${label}/under`, under);
  inspect(`${label}/hyphen`, hyphen);
  inspect(`${label}/dot`, dot);
  queryCounts(label);
  say(`${label} :: complete`);
};
...
system.afterEvents.scriptEventReceive.subscribe((ev) => {
  if (ev.id === "probe:hit") { say(`console-selector :: hit ${(ev.message || "").trim()}`); return; }
  if (ev.id === "probe:checked") { say(`console-selector :: checked ${(ev.message || "").trim()}`); return; }
