...
all three. The last of them is the one a reader has to ask for, because the format cannot: whether
an observation depends on a version, a fixture, or some other state is a reading of the claim, and
a run whose result would differ against a different one says which it ran against
[[r:a-run-records-what-its-observation-depends-on]]. Where nothing would differ there is nothing to
record, which is why this is a test rather than a required field.
...
| 5d | run — dependence | Where a run's observation would differ against another version, fixture, or state, does the run say which it ran against? |
