...
| `resting-kinematics-isolated` | the same question as `resting-kinematics`, sampled so the post-spawn readings are usable: one type at a time, spawned 4 blocks clear of the source and removed before the next, sampled on the spawn frame and again at 2 and 22 ticks. Each sample carries `neighboursWithin4`, so a contended reading is visible in the log rather than assumed absent. `resting-kinematics` holds all eight types live at one point — with the source entity in the pile — so its post-spawn samples read overlap resolution, not rest |
