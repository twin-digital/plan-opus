...
questions:
  - id: archive-interface-wording
    ...
    closes: requirement
    gates: [the-archive-ships-as-a-bin-command]
```
...
it; the package directory is that url's directory [[d:the-consumer-hands-over-its-config-file-url]].
It returns a partial tsdown config setting exactly these keys:
...
[[f:tsdown-takes-rolldown-plugins-through-its-config]], and because declaring the extra rebuild
inputs a pack has is something only a plugin can do
...
build produces, and the kit package's README carries a usage section holding the fragment above
...
packageName } })` [[d:the-workspace-root-is-found-by-walking-up]]. What comes back is the pack set:
one entry per pack of this package, each carrying the pack's kind, its source location, its build
output location, and the full content of its manifest with the fields completion populates
...
A pack the kit marks invalid fails the build: the plugin throws, printing the pack's name and its
structured problems, and builds neither that pack nor its sibling
[[d:an-invalid-pack-fails-the-build]]. A package for which the set is empty fails the same way,
naming the package directory that was searched [[d:a-package-with-no-pack-fails-the-build]].
...
marks exactly those specifiers, and subpath imports of them, external. An import that resolves into
a package under the `@minecraft/` scope the manifest does not declare fails the build naming the
specifier and the importing file, because bundling one of those modules produces a pack that fails
at the server instead [[d:externals-come-from-the-manifests-module-dependencies]].
...
manifest, bundle, copied files — and at the end walks `<package>/dist/<kind>/` and deletes anything
absent from that set, then removes the directories left empty. It never clears the tree before
building [[d:stale-output-is-pruned-not-wiped]]. Every write is compared against what already sits
...
graph never sees them change. The plugin declares them: at `buildStart` it calls `addWatchFile` on
every file under each pack's source directory, on `<package>/behavior_pack/manifest.json` and
`<package>/resource_pack/manifest.json` where they exist, and on `<package>/package.json`
[[f:a-rolldown-plugin-can-declare-extra-watch-inputs]] [[r:rebuild-triggers-are-declared-not-inferred]].
...
change rebundles and the bundle is rewritten. A change to a manifest input alone — a version bump, a
manifest edit — re-completes the manifests and rewrites them, while the bundle's bytes are unchanged
and so are not written at all [[d:output-files-are-written-only-when-their-bytes-change]]. A version
bump raised in a sibling package that this pack depends on is picked up on the next build rather
than triggering one.
...
components:
  ...
  - id: build-plugin
    responsibility: |
      ...
      resolution, and the order the writers run in
