...
- **Search** — find packs by npm package name, pack name, pack uuid, or entry status, each matched exactly. Criteria combine, and a search that does not constrain status returns valid entries only.
...
Enumerating the workspace's packages was the one place worth weighing, and the evidence gathered since settles most of it: both managers publish an enumeration library from their own repository, and both libraries list an uninstalled workspace without being handed a manager binary, so nothing here needs shelling out to a manager or re-implementing its configuration formats and glob behaviour. What is still open is the cost, in two places. A member package whose `package.json` will not parse makes both libraries throw and return nothing, so a fault in one package — possibly one with no pack in it at all — fails the whole call. And a directory holding `behavior_pack/manifest.json` inside a folder with no `package.json` of its own is skipped by both libraries without error, so it appears nowhere in the list at all — the one hole in the exhaustiveness the Discover bullet promises. Whether each of those is acceptable, or a fault has to stay with the package it belongs to and a skipped pack has to be reported, is the question this hands the spec.
